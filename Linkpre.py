...
parser.add_argument('--reg1', type=float, default=0.01, help='reg1')
parser.add_argument('--reg2', type=float, default=0.01, help='reg2')
...
def gae_for(args):
    ...
    for epoch in range(args.epochs):
        ...
        # loss = loss_function(preds=re, labels=adj_label, norm=norm, pos_weight=pos_weight)
        # loss.backward()
        ...
        '''
        G3_train_loss = -torch.mean(log(below_loss))
        G3_train_loss.backward(retain_graph=True)
        g3_loss = G3_train_loss.item()
        optimizer.step()
        '''
        ##################
        ...
        val_roc, val_ap = get_roc_score(hidden_emb, adj_orig, val_edges, val_edges_false)
        #NMI.append(nmi)
        #AC.append(ac)
        AUC.append(val_roc)
        AP.append(val_ap)
        ...
        print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(cur_loss),
              g_loss, d_loss, "val_roc=", "{:.5f}".format(val_roc),
              "val_ap=", "{:.5f}".format(val_ap),
              "time=", "{:.5f}".format(time.time() - t)
              )
        lastroc, lastap_score = get_roc_score(hidden_emb, adj_orig, test_edges, test_edges_false)
        print('Test ROC score: ' + str(lastroc))
        print('Test AP score: ' + str(lastap_score))

    print("Optimization Finished!")
    # x = list(range(args.epochs))
    # line = pyecharts.Line(f'NMI on {args.dataset_str}')
    # line.add('NMI', x, NMI, mark_point=['max', 'min'], mark_line=['average'])
    # line.render(f'{args.dataset_str}-nmi.html')
    ...
    Linkemb = embedding[AUC.index(max(AUC))]
    roc_score, ap = get_roc_score(Linkemb, adj_orig, test_edges, test_edges_false)
    print('Test ROC score: ' + str(roc_score))
    print('Test AP score: ' + str(ap))
